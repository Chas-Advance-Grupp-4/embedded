/**
 * @file GetHandler.h
 * @brief Abstract base class for handling HTTPS GET requests in the REST server.
 *
 * Provides a reusable structure for defining GET endpoints using the ESP-IDF
 * HTTPS server. Subclasses must implement `process()` to define the response
...
/**
 * @class GetHandler
 * @brief Abstract base class for HTTPS GET route handlers.
 *
...
class GetHandler : public BaseHandler {
  ...
  private:
     ...
     *
     * Required by ESP-IDF's C-style HTTPS server, which expects a plain C
